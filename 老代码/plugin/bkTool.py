...
class BkTool:
    def __init__(self):
        self.web = WebRequest()
        self.url = config.read("BK", "url") + "/"
        self.user = config.read("BK", "user")
        self.pwd = config.read("BK", "passwd")

    def login(self):
        # 登录
        try:
            url = self.url + "auth/sign-in"
            method = "POST"
            header = ToolUtil.GetHeader(url, method)
            header.pop("authorization")

            data = {"email": self.user, "password": self.pwd}
            data = json.dumps(data)
            success = self.web.post(url, data, header=header, timeout=30, retry_interval=3, retry_limit=3, use_proxy=False)
            if success:
                req_data = json.loads(self.web.content)
                token = req_data["data"]["token"]
                logger.info(f"bk更新登录token: {token}")
                config.write("BK", "token", token)
            else:
                logger.warning(f"bk登录失败 {self.web.text}")
        except Exception as e:
            logger.error(f"bk无法刷新登录token, {e}")

    def getIndexPages(self):
        # 获取一共有多少页
        try:
            url = self.url + "comics?page=1&s=dd"
            method = "GET"
            header = ToolUtil.GetHeader(url, method)

            success = self.web.get(url, header=header, timeout=30, retry_interval=3, retry_limit=3, use_proxy=False)
            if success:
                data = self.web.json

                pages = data["data"]['comics']["pages"]
                total = data["data"]['comics']["total"]

                return pages, total

        except Exception as e:
            logger.error(f"bk获取index分页数量失败: {e}")
        return None, None
    ...
    def getImageSplitPages(self, cid, order):
        # 获取一个章节一共有多少个图片分页
        try:
            url = self.url + f"comics/{cid}/order/{order}/pages?page={1}"
            method = "GET"
            header = ToolUtil.GetHeader(url, method)
            success = self.web.get(url, header=header, timeout=30, retry_interval=3, retry_limit=3, use_proxy=False)
            if success:
                ...
                total = data["data"]["pages"]["total"]
                return pages, total

        except Exception as e:
            logger.error(f"bk获取章节分页数量失败: {e}")
        return None, None

    def getPage(self):
        # 获取一个章节的图片信息
        bookId = "62e6285e26e48d097bc808eb"
        epsId = 1
        page = 1
        url = self.url + f"comics/{bookId}/order/{epsId}/pages?page={page}"
        method = "GET"
        header = ToolUtil.GetHeader(url, method)
        success = self.web.get(url, header=header, timeout=30, retry_interval=3, retry_limit=3, use_proxy=False)
        print(self.web.text)
...
class ToolUtil(object):
    ...
    def GetHeader(cls, _url: str, method: str) -> dict:
        ...
        url = config.readStr("BK", "url") + "/"
        Agent = config.readStr("BK", "agent")
        ApiKey = config.readStr("BK", "api_key")
        Version = config.readStr("BK", "version")
        BuildVersion = config.readStr("BK", "build_version")
        Accept = config.readStr("BK", "accept")
        AppChannel = config.readStr("BK", "app_channel")
        Uuid = config.readStr("BK", "uuid")
        ImageQuality = config.readStr("BK", "image_quality")
        Platform = config.readStr("BK", "platform")
        UpdateVersion = config.readStr("BK", "update_version")
        datas = [
        ...
        header["authorization"] = config.read("BK", "token")
...
再試試看吧"""
    ...
    def SaveFile(data, filePath):
        ...
        try:
            ...
            log.logPrint("add chat cache, cachePath:{}".format(filePath))
...
if __name__ == '__main__':
    ...
    bk.login()
    # bk.getIndexPages()
    bk.getImageSplitPages("6461d92b2b80b519055448a5", 12)