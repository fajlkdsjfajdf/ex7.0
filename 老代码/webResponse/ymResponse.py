...
class YmResponse(Response):
    ...
    def bindBiliManga(self, aid, url):
        try:
            if url and aid:
                bm = BilibliManga()
                ...
                if bilibili_id:
                    ...
                    if bilibili_list:
                        info = {
                            ...
                            "url": url,
                            "list": bilibili_list,
                            "update": timeFormat.getNowTime()
                        }
                        self.db.processItem(f"{self.prefix.lower()}-bind", info, "aid")
                        return {"msg": "绑定完成"}, "json"
                    else:
                        logger.error(f"绑定bilibli漫画失败")
                        return {"msg": "绑定失败"}, "json"
                else:
                    return {"msg": "绑定失败,输入的url不正确"}, "json"
    ...
    def bindData(self):
        ...
        return self.bindBiliManga(aid, url)
    ...
    def getForum(self):
        ...
        if list_data:
            ...
            if bind_data and order > 0:
                ...
                if(bind_item):
                    if "update" in bind_item and timeFormat.getNowTime() - bind_item["update"] > timedelta(days=1):
                        # 刷新列表
                        url = bind_item["url"]
                        self.bindBiliManga(aid, url)
