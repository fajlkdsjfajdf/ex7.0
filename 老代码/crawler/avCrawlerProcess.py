...
# ************************************** list爬虫 **************************************
class AvCrawlerProcess(CrawlerProcess):
    ...
    def _getCrawlerData(self):
        ...
        data1 = [{"page": p, "type": ""} for p in range(1, 100 + 1)]

        data2 = [{"page": p, "type": "47251408c7a81957"} for p in range(1, 100 + 1)]    # 裤袜
        return data1 + data2
    ...
    def _getUrl(self, crawler_data_index):
        ...
        if search:
            ...
            if search_type == "":   # 有码搜索
                url = f"{self.url}/cn/page/{page}"
            else:
                url = f"{self.url}/cn/genre/{search_type}/page/{page}"
        return url
    ...
    def _webGet(self, web, url, crawler_data_index):
        ...
        return web.get(url, header=self.header, cookie=self.cookie, web_index=crawler_data_index,
                               timeout=30, retry_interval=3, retry_limit=3, use_proxy=True)
    ...
    def _getInfo(self, web, crawler_data_index):
        try:
            data = []
            tree = web.tree
            for item in tree.xpath(".//div[@id='waterfall']/div[@class='item']"):
                url = ''.join(item.xpath("./a/@href")).strip()
                if "movie" in url:
                    aid = url.split("/")[-1]
                    data.append({"aid": aid, "index_update": datetime.now()})
            if(data):
...
# ************************************** info爬虫 **************************************
class AvInfoCrawlerProcess(AvCrawlerProcess):
    ...
    def _getUrl(self, crawler_data_index):
        data = self.crawler_data[crawler_data_index]
        return f"{self.url}/cn/movie/{data['aid']}"
    ...
    def _getInfo(self, web, crawler_data_index):
        try:
            ...
            data_dict['title'] = ''.join(tree.xpath(".//h3/text()")).strip()
            data_dict['fanhao'] = ''.join(tree.xpath(
                ".//div[@class='col-md-3 info']//span[contains(text(),'识别码')]/following-sibling::span[1]/text()")).strip()
            data_dict['Length'] = ''.join(tree.xpath(
                ".//div[@class='col-md-3 info']//span[contains(text(),'长度')]/../text()")).replace("分钟", "").strip()
            try:
                data_dict['ReleaseDate'] = datetime.strptime(''.join(tree.xpath(
                    ".//div[@class='col-md-3 info']//span[contains(text(),'发行时间')]/../text()")).strip(), '%Y-%m-%d')
            except:
                data_dict['ReleaseDate'] = datetime.strptime("1900-01-01", '%Y-%m-%d')
            data_dict['Director'] = ''.join(tree.xpath(
                ".//div[@class='col-md-3 info']//span[contains(text(),'导演')]/../a[1]/text()")).strip()
            data_dict['Studio'] = ''.join(tree.xpath(
                ".//div[@class='col-md-3 info']//p[contains(text(),'制作商')]/following-sibling::p[1]/a[1]/text()")).strip()
            data_dict['Label'] = ''.join(tree.xpath(
                ".//div[@class='col-md-3 info']//p[contains(text(),'发行商')]/following-sibling::p[1]/a[1]/text()")).strip()
            data_dict['Series'] = ''.join(tree.xpath(
                ".//div[@class='col-md-3 info']//p[contains(text(),'系列')]/following-sibling::p[1]/a[1]/text()")).strip()
            data_dict['pic_l'] = ''.join(tree.xpath(".//a[@class='bigImage']/img[1]/@src"))
            data_dict['PicList'] = tree.xpath(".//div[@id='sample-waterfall']//img/@src")
            data_dict['tags'] = tree.xpath(".//span[@class='genre']/a/text()")
            data_dict['info_update'] = datetime.now()
            data_dict['stars'] = []
            for star in tree.xpath(".//a[@class='avatar-box']"):
                star_name = ''.join(star.xpath("./span/text()"))
                star_pic = ''.join(star.xpath("./div/img/@src"))
...
# ************************************** 章节爬虫 **************************************
class AvChapterCrawlerProcess(AvCrawlerProcess):
    pass


# ************************************** 图片列表爬虫 **************************************
class AvPageCrawlerProcess(AvCrawlerProcess):
    pass


# ************************************** 首页图片爬虫 **************************************
class AvThumbCrawlerProcess(AvCrawlerProcess):
    def _httpComplete(self, response, url, kwargs):
        """
        httpComplete
        判断http是否真的完成，即使返回200，有时也会被墙屏蔽
...
# ************************************** 缩略图片爬虫 **************************************
class AvNailCrawlerProcess(AvCrawlerProcess):
    def _httpComplete(self, response, url, kwargs):
        """
        httpComplete
        判断http是否真的完成，即使返回200，有时也会被墙屏蔽
...
# ************************************** 内容图片爬虫 **************************************
class AvImagesCrawlerProcess(AvCrawlerProcess):
    pass


# ************************************** 评论页爬虫 **************************************
class AvCommentsCrawlerProcess(AvCrawlerProcess):
    pass


# ************************************** 下载器 **************************************
class AvDownCrawlerProcess(AvCrawlerProcess):
    pass
