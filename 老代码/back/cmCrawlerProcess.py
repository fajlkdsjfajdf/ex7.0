...
class CmCrawlerProcess(CrawlerProcess):
    ...
    def _getUrl(self, value):
        #
        return f"{self.url}/albums?o=mr&page={self.crawler_data[value]}"
    ...
    def _getInfo(self, web, index):
        ...
        try:
            tree = web.tree
            for div in tree.xpath(".//div[@class='p-b-15 p-l-5 p-r-5']"):
                data_dict = {}
                url = ".".join(
                    div.xpath(".//a[contains(@href, '/album/')]/@href")).strip()
                data_dict["aid"] = int(url[url.rindex('/', 0, len(url) - 1) + 1:].replace("/", ""))
                data_dict["pic"] = "".join(div.xpath(".//img/@data-original")).strip()
                if data_dict["pic"] == "":
                    data_dict["pic"] = "".join(div.xpath(".//img/@src")).strip()
                data_dict["title"] = "".join(
                    div.xpath(".//span[@class='video-title title-truncate m-t-5'][1]/text()")).strip()
                data_dict["albim_likes"] = ".".join(div.xpath(".//span[@class='text-white']/text()")).strip()
                a_likes = typeChange.findnum(data_dict["albim_likes"])
                if "K" in data_dict["albim_likes"] or "k" in data_dict["albim_likes"]:
                    a_likes = a_likes * 1000
                elif "M" in data_dict["albim_likes"] or "m" in data_dict["albim_likes"]:
                    a_likes = a_likes * 1000000
                data_dict["albim_likes"] = int(a_likes)
                data_dict["types"] = typeChange.allStrip(
                    div.xpath(".//div[@class='label-category'  or @class='label-sub']/text()"))
...
class CmCommentsCrawlerProcess(CmCrawlerProcess):
    ...
    def _webGet(self, web, url, index):
        ...
        return web.post(url, info, header=self.header, cookie=self.cookie, web_index=index,
                        timeout=10, retry_interval=3, retry_limit=5, use_proxy=use_proxy,
                        proxy_state=ProxyState.INCLUD_DOMESTIC)
