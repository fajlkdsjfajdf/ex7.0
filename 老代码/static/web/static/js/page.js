...
async function getPage(index){
    ...
    window.page_title = "";
    //$(".loading").show();
    html = await page_fun[window.prefix](index);
...
const page_fun = {
    ...
    bs: async function(index){
        function buildText(data){
            ...
            for(let page in data){
                html += `<div page="${page}" class="page_div"><h2 class="title">第${page}页</h2> ${data[page]}</div>`;
                //html += data[page]
            }
        ...
        if(window.info_data["lists"].length > (index+1)){
            ...
            console.log("同步加载下一章");
            let nid = window.info_data["lists"][index+1]["_id"];
            let aid = window.info_data["lists"][index+1]["aid"];
            let n_pid = window.info_data["lists"][index+1]["pid"];
            let n_order = window.info_data["lists"][index+1]["order"];
            if(`7654${aid}${n_order}` == n_pid.toString() && ("new_pid" in window.info_data["lists"][index+1])== false){
                //没有new_pid不做事
    ...
    km: async function(index){
        function getkmImages(images){
            let new_images = [];
            for(let i in images){
                let img = images[i];
                let src = window.host_url + "/api/image/" + img["kid"];
                new_images.push(src);
            }
            return new_images;
        }
        ...
        let aid = window.info_data["aid"];
        let pid = page_data["pid"];
        ...
        if("images" in page_data){
            let id = page_data["_id"];
            let images = page_data["images"];
            images = getkmImages(images);

            headers = {"referer": `${window.host_url}/comic/${aid}/chapter/${pid}/images/all`};
            return buildCachaImages(id, images, headers, {}, window.proxies_mode);
        }
        else{
    ...
    ym: async function(index){
        ...
        if(window.info_data["lists"].length > (index+1)){
            ...
            console.log("同步加载下一章");
            let nid = window.info_data["lists"][index+1]["_id"];
            if("images" in window.info_data["lists"][index+1] == false){
                window.web_tools.startPage([nid], ["page"], false);
            }
        }
        ...
        if("images" in page_data && page_data["images"].length > 0){
            let id = page_data["_id"];
            let images = page_data["images"];
            headers = {"referer": `${window.host_url}`};
            return buildCachaImages(id, images, headers, {}, window.proxies_mode);
        }
        else{
            $(".loading").show();
            ...
            await window.web_tools.startPage([lid], ["page"], function(data){
                ...
                if(info.length > 0){
                    ...
                    if(info[0]["complete"] == "True" && "images" in info[0]){
                        ...
                        let images = info[0]["images"];
                        headers = {"referer": `${window.host_url}`};
                        let html = buildCachaImages(id, images, headers, {}, window.proxies_mode);
                        $("#page .imgs").append(html);
                        buildSlider();
                        add_pages_over();
                    }
...
function buildCachaImages(id, images, headers={}, cookies={}, use_proxies=0){
    let pages = images.length;
    let html = "";
    for (let i=0;i<pages;i++) {
        let src = images[i];
        src = getCacheImgSrc(src, headers, cookies, use_proxies);
        html += `<img src="/static/images/wait.gif" data-original="${src}" page="${i + 1}" id="${id}" class="lazy">`;
    }
    return html;
}
...
function goPageNum(page_num){
    try{
        let img = $("#page .imgs img[class='lazy'], #page .page_div").eq(page_num - 1);
        if(img && img.offset()){
...
function buildSlider(){
    let max = $("#page .imgs img[class='lazy'], #page .page_div").length;
...
async function getForum(update=true){
    ...
    else if (["cm", "mg", "km", "ym"].includes(window.prefix)){
        id = window.img_page_id;
    }
...
function reload_img(img){
    ...
    if(src.indexOf("imgcache")>=0){
        console.log("重载图片");
        let new_src = src + "&wts=" + Date.now();
...
function add_pages_over(){
    ...
    if(window.prefix == "bs"){
        ...
        $(".page_div").find('p').on("dblclick", function() {
            $(this).toggleClass("garbled-text");
        });

        // $(".page_div").find('p').each(function() {
        //     const $p = $(this);
        //     const text = $p.text();
        //
        //
