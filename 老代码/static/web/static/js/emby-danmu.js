    ...
    function createDanmaku(comments) {
        ...
        window.ede.timeOb = new MutationObserver(() => {
            ...
            if(window.ede.playerTime != playerTime){
                ...
                if(playerTime in _comments){
                    let comment_list = _comments[playerTime];
                    for(let i=0;i<window.ede.danmuLimit;i++){
                        if(i in comment_list){
                            window.ede.danmaku.send(comment_list[i]);
                        }
                    }

                }
                //设置高级(顶部)弹幕，每秒只允许1条
                if((playerTime+"_top") in _comments){
                    let comment_special = _comments[playerTime+"_top"][random(0, _comments[playerTime+"_top"].length)];
