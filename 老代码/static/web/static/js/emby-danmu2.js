...
(function (global, factory) {
  ...
  function allocate(cmt) {
    ...
    return channel % (this._.height / 3 - cmt.height);
  }
  ...
  function createEngine(framing, setup, render, remove) {
    return function() {
      ...
      while (this._.position < this.comments.length) {
        ...
        ++this._.position;
        //let danmuCount = $(".danmubox div").length;
        //if(danmuCount >= this._limit)
            //break;
  ...
  return Danmaku;
    ...
    function infoButtonClick() {
        console_log('显示当前信息');
        if (window.ede.episode_iResizingnfo != null) {
    ...
    function initUI() {
        ...
        // 简繁转换
        // ranslateButtonOpts.title = chConverTtitle[window.ede.chConvert];
    ...
    async function getEpisodeInfo(is_auto = true) {
        ...
        let searchUrl = 'http://ainizai0904.asuscomm.com:8071/danmuku?type=search&keyword=' + animeName + '&withRelated=true';
        if (is_auto) {
    ...
    function getComments(episodeId) {
        let url = 'http://ainizai0904.asuscomm.com:8071/danmuku?type=comment&episodeId=' + episodeId + '&withRelated=true&chConvert=' + window.ede.chConvert;
        return fetch(url)
