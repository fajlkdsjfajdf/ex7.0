...
window.thumb_check = true; // 是否开启thumb检查
    ...
    if(window.page_load == false || retry){
        ...
        if(data == null){
            ...
            for(let d in list_data){
                let item = list_data[d];
                item = list_fun[prefix](item);
...
const list_fun = {
    ex: function(data){
        ...
        let comments_info = getDefaultFromDict(data, "comments_info", []);
        d["comment"] = "";

        if(comments_info.length > 0){
            d["comment"] = `</br>${comments_info.length}条 ${comments_info[0]} `;
        }
        return d;
    },
    mh: function(data){
    ...
    ym: function(data){
        ...
        d["list_title"] = getDefaultFromDict(data, "chapter_last", "");
        d["rating"] = getDefaultFromDict(data, "rating", 0);
        d["date"] = "";
        d["serial_status"] = getDefaultFromDict(data, "chapter_status", "");
        let tags = getDefaultFromDict(data, "tags", []);

        d["type1"] = d["serial_status"];
        d["type2"] = "";
        return d;
    },
    av: function(data){
    ...
    cm: function(data){
        ...
        if(d["listcount"] > 1){
            ...
            let fj =  getDefaultFromDict(end_list, "order", "");
            list_title = fj.toString() + "章 " + list_title;
        ...
        if(comments_info.length > 0){
            d["comment"] = `</br>${comments_info.length}条 - pid: ${comments_info[0]["pid"]} - ${comments_info[0]["comment"]} `;
        }
        return d;
    },
    cb: function(data){
