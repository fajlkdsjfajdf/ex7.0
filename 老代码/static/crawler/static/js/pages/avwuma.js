...
function setStepButton(){
    ...
    $(".bgmsearch").click(async function(){
        ...
        let url = "/status?cls=ZhengliStatus&type=searchwuma&search_text={text}&search_type=text";
...
async function avDataGet(index){
    ...
    let id = item["_id"];
    let url = "/status?cls=ZhengliStatus&type=wumaavlink";
    let data = await fetchApi(url, "POST", {"item": item, "path": av_path});
    alert(data);
}
