...
$( document ).ready(function() {
    ...
    $(".bgmidsearch").on("click", function(){
        let bid = $(".searchid").val();
        if(bid){
            startZhengli(bid);
        }
...
async function getLfData(){
    let option = {
        "cls": "ZhengliStatus",
        "type": "getlffiles"
    };
    let data = await fetchApi("/status", "GET", option);
    //console.log(data);
    window.lf_data = data;
...
function findSameSet(d){
    let set_name = d["set"]["name"];
    let title = d["title"];
    let new_data = [];
    let same_v_c = $("#samev-text").val();
    for(let i in window.lf_data){
        let item = window.lf_data[i];
        if("set" in item){
            if(item["set"]["name"] == set_name){
                new_data.push(item);
            }
            else{
                let use_title = $('input[name=choice2]:checked');
                //console.log(use_title);
                if(use_title.length> 0){
                    //标题相似度匹配
                    let title2 = item["title"];
                    let same_v = getSimilarity(title, title2);
...
async function lifansearch(index){
    //console.log(index);
    let item = window.lf_data[index];
    let set_name ="";
    if("set" in item){
        let data = findSameSet(item);
        addLifanSearch(data);
        console.log(data);
    ...
    let search_arr = originaltitle.split("/");
    if(set_name != ""){
        search_arr.push(set_name);
    }
...
function addLifanSearch(data){
    ...
    for(let i in data){
        let file = data[i];
        let file_name = file["org_file"].split('/').pop();
        let id = data[i]["_id"];
        $(".files").append(stringFormatByDict(html, {"parent": "", "file": file_name, "size": "", "path": file["org_file"], "index": i, "_id": id}));
    }
    bindingZhengliButton();

}

async function linkDecode(){
    let url = "status?cls=ZhengliStatus&type=linkdecode";
...
function showSearchModal() {
    ...
    $(".search_action").empty();
    //添加查询备选
    let search_arr = window.search_arr;
    let search = "";
    for(let i in search_arr){
        ...
        li.on("click", function(){
            $(".searchname").val(li.html());
        });
        $(".search_action").append(li);
    }
...
async function startZhengli(bid){
    ...
    // const userConfirmation = confirm("是否确认继续？");
    // if (userConfirmation) {
    //   // 用户点击了“确定”按钮，执行后续操作
    // } else {
    //   // 用户点击了“取消”按钮，不执行后续操作
    ...
    let data = {"bid": bid, "token": bgm_token};
    data["zl"] = [];
    let ids = [];
    $("#video_change a").each(function(){
        let old_path = $(this).attr("oldpath");
        let path = $(this).attr("path");
        let id = $(this).attr("_id");
        if(old_path && path){
            ids.push(id);
            let d = {"old_path": old_path, "path": path, "id": id};
            data["zl"].push(d);
        }
    ...
    let info = await fetchApi("status?cls=ZhengliStatus&type=lfzl", "POST", data);
    console.log(info);
    if("status" in info && info["status"] == "success"){
        toastr.success("完成");
        for(let i in ids){
            let id = ids[i];
            $("td[id='"+id+"']").attr("class", "check");
        }
        $('#myModal').modal('hide');
    }
...
function bindingZhengliButton(){
    ...
    $(".from").click(function(){
        jishu = 0;
        ova_ji =0;
        $("a[type='from']").attr("type", "");
        $("a[type='end']").attr("type", "");
        $(this).parent().attr("type", "from");
        let index = $(".video").index($(this).parent());
        console.log(index);
        $("#video_change").empty();

        for(let i=0;i<index;i++){
            let a =  $(".video").eq(i).clone(true);
    ...
    $(".end").click(function(){
        $("a[type='end']").attr("type", "");
        $(this).parent().attr("type", "end");
        let from = $("a[type='from']");
        let end = $("a[type='end']");
        let list = from.nextUntil("a[type='end']");
        //console.log(list);
        $(".video").removeClass("active");
        list.addClass("active");
        from.addClass("active");
        end.addClass("active");
        title_change($(".ji-num").val());
    });
    ...
    $(".add").click(function(){
        ...
        jishu = jishu + 1;
        let title = $(this).parent().attr("path");
        let index = title.lastIndexOf("/");
        let old_path = $(this).parent().attr("path");
        title = title.substring(index + 1,title.length);
        let ji = pad(parseInt($(".ji-num").val()), 1) ;
        ...
        let a = $(get_end_video_str(old_path, video, title, id));
    ...
    $(".ova").click(function(){
        ...
        let ji = pad(parseInt($(".ji-num").val()), 1) ;
        let num = pad(parseInt(ova_ji), 2);
        let ove_num = ji +""+ num;

        let pre_title =  "S0E"+ove_num + " " ;
        let video = "";
        if(title.indexOf(pre_title)>=0){
            video = "S0"+  "/" + title;
        }
        else{
            video = "S0"+  "/" + pre_title + title;
        }
...
function get_end_video_str(old_path, video, oldfilename, _id){
    return '<a _id="'+_id+'" href="javascript:void(0);" oldfilename="'+oldfilename+'" oldpath="'+ old_path + '" path="'+video+'" class="list-group-item video_change link"><h4 class="list-group-item-heading">' + video +  '</h4><button class="from">...</button></a>'
}
    ...
    $(".bgmsearch").click(async function(){
        ...
        for(d in data){
            ...
            if ("name_cn" in data[d])
                chinese = data[d]["name_cn"];
            if(data[d]["type"] == 2)
                html = '<tr style="background: antiquewhite">';
            let bid = data[d]["id"];
            let url = "https://bgm.tv/subject/" + bid;
            html += "<td><img style='width:150px;' src='"+ img +"' /></td><td>"+ title+"</td><td>"+chinese+"</td>" ;
            html +="<td><button class='btn btn-default' onclick='go_url(\""+ url  +"\")'>查看</button></td>" ;
